...
// Problem 1: Big Donors
let bigDonors = [];
bigDonors = runners.filter((runner) => runner.donation > 200);
console.log(bigDonors);
// Problem 2: Government Donors
let govDonors = [];
govDonors = runners.filter((runner) => runner.email.includes("gov") === true);
console.log(govDonors);
// Problem 3: Small Donations
let smallDonations = 0;
let smallDonors = [];
smallDonors = runners.filter((runner) => runner.donation < 10);
smallDonations = smallDonors.reduce((acc, runner) => {
    return acc + runner.donation;
}, 0)
